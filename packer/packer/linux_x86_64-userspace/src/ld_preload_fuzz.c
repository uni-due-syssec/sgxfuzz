...
void report_struct_synth_addrs(const char* addrs, ssize_t len) {
...
static void hprintf_payload(char* data, size_t len){
...
ssize_t call_vm(void *data, size_t max_size, bool return_pkt_size){
...
ssize_t read(int fd, void *data, size_t size) {
  ...
  if(fuzzer_ready){
    if(fd == 0){
        ...
        ssize_t return_value = call_vm(data, size, false);
...
ssize_t getline(char **lineptr, size_t *n, FILE *stream){
  if(fileno(stream) == 0){
    *lineptr = malloc(128);
    //printf("%p\n", *lineptr);
    ssize_t size = call_vm(*lineptr, 126, false);
    ASSERT(size <= 126);
    if(size == -1){
      return -1; /* EOF*/
    }
...
void nyx_init_start(void){
    ...
    */
    ...
    if(ar->found){
        ...
        hprintf("=> IP0:\t\t0x%016lx-0x%016lx YOLO\n", ar->ip0_a, ar->ip0_b);
        hprintf("=> IP1:\t\t0x%016lx-0x%016lx YOLO\n", ar->ip1_a, ar->ip1_b);
    }
...
*/
    ...
    if(get_harness_state()->afl_mode){
        ...
        if(!get_harness_state()->delayed_init && !get_harness_state()->net_fuzz_mode){
            hprintf("delayed_init mode disabled!\n");
            capabilites_configuration(false, false, true);
            nyx_init_start();
        }    
        else{
            hprintf("delayed_init mode enabled!\n");
        }
    }
    return original__libc_start_main(main,argc,ubp_av, init,fini,rtld_fini,stack_end);
}