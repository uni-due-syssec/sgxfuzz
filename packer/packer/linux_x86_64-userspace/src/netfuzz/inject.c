...
extern ssize_t call_vm(void *data, size_t max_size, bool return_pkt_size);
...
char* handle_next_packet_s(int client_socket, char* s, int size){
	if(client_socket != -1){
		...
		for(int i = 0; i < size-1; i++){
			...
			DEBUG("%s -> %d call_vm\n", __func__, i);
			ssize_t ret = call_vm(s+i, 1, false);
...
int handle_next_packet(int client_socket, void* buf, size_t len, bool return_pkt_size){
	if(client_socket != -1){
		...
		ssize_t data_len = call_vm(buf, len, return_pkt_size);
...
int handle_next_packet_iovec(int client_socket, const struct iovec *iov, int iovcnt, bool return_pkt_size){
  ...
  static char data_buffer[PACKET_BUFFER_SIZE];
	...
	if(client_socket != -1){
		...
		ssize_t data_len = call_vm(data_buffer, len, return_pkt_size);
...
int connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen){
	...
	if(!exists && ntohs(addr_in->sin_port) == 1337){
...
int epoll_wait(int epfd, struct epoll_event *events, int maxevents, int timeout){
	...
	for(int i = 0; i < maxevents; i++){
		if(u64_tmp){
			hprintf("CHECKING %lx vs %lx\n", u64_tmp, events[i].data.u64);
			if(u64_tmp == events[i].data.u64){
				hprintf("========>>>>>> %d FOUND\n", fd_tmp);
			}
		}
	}
