...
class GuiDrawer:
    ...
    def draw(self):
        ...
        self.gui.print_info_line([
            #(16, "", ""),
            (16, "Progress", pfloat(d.edges_progress())),
        ...
        print_slaves = d.num_slaves()
        if 40 <= rows < print_slaves + 36:
            print_slaves = rows - 36
        print_start = 0
        if print_slaves < d.num_slaves():
            print_start = min(max(0, self.current_slave_id - 4), d.num_slaves() - print_slaves)
        for i in range(print_start, print_start + print_slaves):
            hl = " "
            ...
            if d.slave_is_stalled(i):
                ...
                struct_ = d.nodes[nid].get("struct", {})
                struct_info = []
                if "ecall" in struct_:
                    struct_info.append(f"E{struct_['ecall']}")
                if "data" in struct_:
                    struct_info.append(struct_["data"].decode().strip())
                struct_info = " ".join(struct_info)
                max_struct_width = max(max_struct_width, len(struct_info))
                column_data = [
                    (15, "", d.slave_stage(i)),
                    (10, "node", "%5d" % d.slave_input_id(i)),
                    ...
                    (max(15, self.max_struct_width + 2), struct_info, ""),
                ]
                self.column = max(0, min(self.column, len(column_data) - 1))
                if self.column > 0:
                    column_data = [(3, "", "...")] + column_data[self.column:]

                self.gui.print_info_line(column_data, prefix="%c Slave %2d" % (hl, i))
            else:
                self.gui.print_info_line([
                    (15, "", d.slave_stage(i)),
                    (10, "node", "    -"),
        ...
        if max_struct_width != self.max_struct_width:
            self.gui.clear()
    ...
    def loop(self):
        ...
        while True:
            ...
            if char == curses.KEY_UP:
                ...
                self.current_slave_id = max(0, self.current_slave_id - 10)
                redraw = True
            elif char == curses.KEY_NPAGE:
                self.current_slave_id = min(self.current_slave_id + 10, self.data.num_slaves() - 1)
                redraw = True
            elif char == curses.KEY_HOME:
                self.current_slave_id = 0
                redraw = True
            elif char == curses.KEY_END:
                self.current_slave_id = self.data.num_slaves() - 1
                redraw = True
...
class GuiData:
    ...
    def execs_p_sec_cur(self):
        return sum([x.get("execs/sec", 0) for i, x in enumerate(self.slave_stats) if not self.slave_is_stalled(i)])
    ...
    def edges_progress(self):
        try:
            return self.stats.get("bytes_worked")/self.total_execs()
    ...
    def yield_havoc(self):
        return (self.stats["yield"].get("afl_havoc", 0) +
                ...
                self.stats["yield"].get("afl_damage", 0) +
    ...
    def read_file(self, name):
        ...
        while retry > 0:
            try:
                ...
                retry -= 1
        if data:
            return clean_data(msgpack.unpackb(data, raw=False, strict_map_key=False))
        else:
            return None
