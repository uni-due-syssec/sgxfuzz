...
class TraceParser:
    ...
    def parse_trace_file(trace_file):
        if not os.path.isfile(trace_file):
            logging.info("Could not find trace file %s, skipping.." % trace_file)
            return None
    ...
    def coverage_totals(self):
        ...
        logging.info(" Processed %d traces with a total of %d BBs (%d edges)." \
    ...
    def gen_reports(self):
        ...
        logging.info(" Plot data written to %s" % plot_file)
        logging.info(" Unique edges written to %s" % edges_file)
...
def kafl_workdir_iterator(work_dir):
    ...
    for input_file in glob.glob(work_dir + "/corpus/[rctk]*/*"):
        ...
        try:
            ...
            print(f"Failed to read/unpack metadata file '{meta_file}', skipping....")
            continue
...
def get_inputs_by_time(data_dir):
    ...
    if (os.path.exists(data_dir + "/fuzzer_stats") and
        ...
        logging.info("Unrecognized target directory type «%s». Exit." % data_dir)
        sys.exit()
...
def graceful_exit(workers):
    ...
    logging.info("Waiting for Worker to shutdown...")
    time.sleep(1)

    while len(workers) > 0:
        for w in workers:
            if w and w.exitcode is None:
                logging.info("Still waiting on %s (pid=%d)..  [hit Ctrl-c to abort..]" % (w.name, w.pid))
                w.join(timeout=1)
            else:
                workers.remove(w)
...
def generate_traces(config, input_list):
    ...
    if not config.argument_values['ip0']:
        logging.warning("No trace region configured!")
        return None
    ...
    for input_path, _, _ in input_list:
        ...
        if os.path.exists(trace_file):
            logging.info("Skip input with existing trace: %s" % input_path)
        else:
            input_files.append(input_path)

    nproc = os.cpu_count()
    chunksize=1000 #ceil(len(input_files)/nproc)
    offset = 0
    workers = list()
...
def read_metadata(config, node_id):
    metadata_file = config.argument_values["input"] + ("/metadata/node_%05d" % node_id)
    return msgpack.unpackb(read_binary_file(metadata_file), raw=False, strict_map_key=False)
...
def generate_traces_worker(config, pid, input_list):
    ...
    config.argument_values["no_pipe_serial"] = True
    config.argument_values["no_pipe_hprintf"] = True
    config.argument_values["delayed_init"] = True
    q = qemu(1337, config, debug_mode=False)
    if not q.start():
        logging.error("%s: Could not start Qemu. Exit." % pname)
        return None
    ...
    try:
        ...
        for input_path in input_list:
            ...
            if os.path.exists(trace_file):
                pbar.update()
                continue
            # node_id = int(input_path[-5:])
            # metadata_file = config.argument_values["work_dir"] + ("/metadata/node__%05d" % node_id)
            metadata = read_metadata(config, int(input_path[-5:]))
            # print("RUNNING:", metadata)

            if simple_trace_run(q, read_binary_file(input_path), metadata):
...
def simple_trace_run(q, payload, metadata):
    assert "struct" in metadata["info"]
    global null_hash
    q.set_payload_prefix(metadata["info"]["struct"]["data"])
    q.set_ecall_id(metadata["info"]["struct"]["ecall"])
...
def main():
    ...
    if config.argument_values['v']:
        init_logger(config.argument_values["work_dir"])

    if config.argument_values["debug"]:
        init_logger(config.argument_values["work_dir"], logging.DEBUG)

    data_dir = config.argument_values["input"]

    null_hash = ExecutionResult.get_null_hash(config.config_values['BITMAP_SHM_SIZE'])

    logging.info("Scanning target data_dir »%s«..." % data_dir)
