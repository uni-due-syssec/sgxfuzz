...
class Graph:
    ...
    def __process_node(self, nodefile):
        ...
        print("%s: Found %3d from %3d using %s [%s] (favs=%d, stage=%s, exit=%s, lvl=%d, perf=%.3f, score=%2d, l=%03.1fK, t=%dmin)" %
                (t_str, node_id, parent, method[:12].ljust(12), sample[:42].ljust(42),
                    ...
                    #len(favs), stage[:8], exit[:1].title(), level, perf*1000, score, node.get("attention_secs",0)))
