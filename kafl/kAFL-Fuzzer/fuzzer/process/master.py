...
class MasterProcess:
    ...
    def send_next_task(self, conn):
        ...
        if self.busy_events >= 10:
            ...
            if mmh3.hash(main_bitmap) == self.empty_hash:
                logging.warning("Coverage bitmap is empty?! Check -ip0 or try better seeds.")
        ...
        while True:
            for conn, msg in self.comm.wait(self.statistics.plot_thres):
                if msg["type"] == MSG_NODE_DONE:
                    ...
                    logging.info("Received results, sending next task..")
                    if msg["node_id"]:
                    ...
                    if self.debug_mode:
                        logging.info("Received new input (exit=%s): %s" % (
                            msg["input"]["info"]["exit_reason"],
                            repr(msg["input"]["payload"][:24])))

                    assert "struct" in msg["input"]["info"]
                    assert "struct" not in msg["input"]
                    node_struct = {"info": msg["input"]["info"], "state": {"name": "initial"}, "struct": msg["input"]["info"]["struct"]}
    ...
    def maybe_insert_node(self, payload, bitmap_array, node_struct):
        ...
        if should_store or node_struct['state']['name'] in ('initial', 'import'):
            node = QueueNode(payload, bitmap_array, node_struct, write=False)
            node.set_new_bytes(new_bytes or {}, write=False)
            node.set_new_bits(new_bits or {}, write=False)
            self.queue.insert_input(node, bitmap)
        elif self.debug_mode:
            if node_struct["info"]["exit_reason"] != "regular":
                logging.info("Payload found to be boring, not saved (exit=%s)" % node_struct["info"]["exit_reason"])
