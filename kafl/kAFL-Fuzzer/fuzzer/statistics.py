...
class MasterStatistics:
    ...
    def print_finding_line(self, node):
        ...
        print(PREFIX + "%s: Got %4d from %4d: exit=%s, new=%2d/%2d, favs=%2d, perf=%1.3f, len=%1.1fK (%s)"
                % (t_str, node_id, parent, exit, new_bytes, new_bits, favs, perf*1000, plen/1024, method[:12]+ENDC))
        self.print_status_line()

    def print_status_line(self):
        execs = self.data["total_execs"] / (time.time() - self.data["start_time"])
        favs_done = self.data["favs_total"] - self.data["favs_pending"]
        print(FLUSH_LINE + "[*] %5d exec/s, %4d edges, %2d/%d favs fuzzed. findings: <%d, %d, %d>" % (
            execs,
            #self.data["paths_total"],
            self.data["bytes_in_bitmap"],
            favs_done,
            self.data["favs_pending"],
    ...
    def event_slave_poll(self):
        ...
        self.data["bytes_worked"] += self.data["bytes_in_bitmap"] * (sum_execs - self.data["total_execs"])
    ...
    def write_plot(self):
        ...
        with open(self.plot_file, 'a') as fd:
            fd.write("%06d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d,%6.0f\n" % (
                ...
                self.data["bytes_worked"],     # current integral of edges over execs
                ))
