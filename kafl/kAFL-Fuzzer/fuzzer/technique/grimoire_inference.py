...
class GrimoireInference:
    ...
    def generalize_input(self, payload, old_node):
        ...
        logging.info("generalizing input {} with bytes {}".format(repr(payload), old_node["new_bytes"]))
        ...
        logging.info("new input: {}".format(repr(self.generalized_to_string(generalized_input))))
    ...
    def add_to_inputs(self, generalized_input):
        ...
        for token in self.tokenize(generalized_input):
            ...
            if token not in self.tokens:
                logging.info("adding token {}".format(repr(token)))
                self.tokens[token] = 0
            self.tokens[token] += 1
