...
class RedqueenInfo:
    ...
    @staticmethod
    def check_redq_val(val):
        try:
            if 0x7ff0_0000_0000 <= int.from_bytes(val, "little") <= 0x7fff_ffff_ffff:
                return False
            if 0x7ff0_0000_0000 <= int.from_bytes(val, "big") <= 0x7fff_ffff_ffff:
                return False
        except:
            pass
        return True

    def check_redq_vals_for_struct_synth(type, lhs, rhs):
        # if b"ELF" in lhs or b"ELF" in rhs:
        if type == "STR" or not RedqueenInfo.check_redq_val(lhs) or not RedqueenInfo.check_redq_val(rhs):
            # print("REDQUEEN, Discarding: ", type, lhs, rhs)
            return False
        return True
