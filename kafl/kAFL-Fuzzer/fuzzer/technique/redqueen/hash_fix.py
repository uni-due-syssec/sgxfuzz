...
from .parser import RedqueenRunInfo, RedqueenInfo
from .cmp import Cmp
...
class HashFixer:
    ...
    def get_cmps(self, data):
        ...
        logging.info("hashfix run in rq mode")
        ...
        logging.info("hashfix run in non rq mode")
        self.qemu.send_disable_redqueen()
        self.qemu.send_payload(timeout_detection=True, apply_patches=True)
        logging.info("hashfix done running, now parsing")
        res = self.parse_redqueen_results(data)
        logging.info("hashfix done parsing")
        return res

    def mark_unfixable(self, cmp):
        logging.info("Unfixable cmp at: %x" % cmp.addr)
    ...
    def try_fix_data(self, data):
        ...
        logging.info("shape of hashes: ")
        for addr in shape:
            logging.info("\t%x: %d" % (addr, shape[addr]))
    ...
    def parse_redqueen_results(self, data):
        ...
        for line in run_info.hook_info.splitlines():
            ...
            if not RedqueenInfo.check_redq_vals_for_struct_synth(type, lhs, rhs):
                return
    ...
    def try_fix_cmp_with(self, shape, fixed_data, cmp, offsets, lhs, rhs, enc):
        logging.info("Trying mutation %s" % (repr((offsets, lhs, rhs, enc))))
        ...
        try:
            if self.try_fix_cmp_offset(shape, fixed_data, cmp, offsets, rhs):
                logging.info("Mutation fixed it")
                return True
            logging.info("Mutation didn't Fix it")
            return False
        except Exception as e:
            logging.error("fixing hash failed %s" % traceback.format_exc())
            raise e

    def try_fix_cmp(self, shape, fixed_data, run_info, cmp):
        known_offsets = self.redqueen_state.get_candidate_file_offsets(cmp.addr)
        logging.info("known offsets for: %x = %s" % (cmp.addr, known_offsets))
    ...
    def try_fix_cmp_offset(self, shape, data, cmp, offsets, repls):
        ...
        if self.does_data_fix_cmp(shape, data, cmp):
            logging.info("found candidate offset %x %s" % (cmp.addr, repr(offsets)))
