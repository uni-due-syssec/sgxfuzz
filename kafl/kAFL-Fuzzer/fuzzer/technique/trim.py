...
def perform_extend(payload, old_node, send_handler, max_len):
    ...
    pad_bytes = upper
    logging.info("TRIM: stream_extend: pad_bytes=%d" % (pad_bytes))
    ...
    if check_trim_still_valid(old_node, old_res, new_res):
        ...
        logging.info("TRIM: stream_extend: dropped funky NUL padding (len=%d, other finds=%d)" % (pad_bytes, num_findings))
        return None
